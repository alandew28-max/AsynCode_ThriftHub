...
private String simpanFotoKeFolderUnggahan(String pathAsli) throws Exception {
    if (pathAsli == null || pathAsli.trim().isEmpty()) {
        return null;
    }

    pathAsli = pathAsli.trim();

    // Kalau database sudah format unggahan/namafile
    if (pathAsli.startsWith("unggahan/") || pathAsli.startsWith("unggahan\\")) {
        return pathAsli.replace("\\", "/");
    }

    Path source = Paths.get(pathAsli);

    if (!Files.exists(source)) {
        throw new Exception("File foto tidak ditemukan: " + pathAsli);
    }

    // Simpan file fisiknya ke src/unggahan
    ...
    String namaFileBaru = "profil_" + System.currentTimeMillis() + ekstensi;

    Path target = folderUnggahan.resolve(namaFileBaru);

    Files.copy(source, target, StandardCopyOption.REPLACE_EXISTING);

    // Yang disimpan ke database tetap pendek
    return "unggahan/" + namaFileBaru;
}


private String ambilPathFoto(String pathDatabase) {
    if (pathDatabase == null || pathDatabase.trim().isEmpty()) {
        return "";
    }

    pathDatabase = pathDatabase.trim();

    // Cek kalau isinya path lengkap
    File fileLangsung = new File(pathDatabase);

    if (fileLangsung.exists()) {
        return fileLangsung.getAbsolutePath();
    }

    // Cek kalau file ada di root project
    File fileDariProject = Paths.get(
            System.getProperty("user.dir"),
            pathDatabase
    ).toFile();

    if (fileDariProject.exists()) {
        return fileDariProject.getAbsolutePath();
    }

    // Cek kalau file ada di src/unggahan
    File fileDariSrc = Paths.get(
            System.getProperty("user.dir"),
            "src",
            pathDatabase
    ).toFile();

    if (fileDariSrc.exists()) {
        return fileDariSrc.getAbsolutePath();
    }
        ...
        if (rs.next()) {
            ...
            if (tanggalLahir != null) {
                ...
                if (foto != null && !foto.trim().isEmpty()) {
                    ...
                    String pathFotoTampil = ambilPathFoto(foto);

                    if (!pathFotoTampil.isEmpty()) {
                        ImageIcon icon = new ImageIcon(pathFotoTampil);
    ...
    try {
        ...
        String fotoUntukDatabase = simpanFotoKeFolderUnggahan(pathFoto);
       ...
       pst.setString(5, fotoUntukDatabase);
